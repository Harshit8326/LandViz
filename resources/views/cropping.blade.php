...
<head>
  <meta charset="UTF-8">
  <title>Cropping Patterns - LandViz</title>
  <style>
    body {
...
<body>
  ...
  <div class="sidebar">
    <div class="sidebar-content">
      ...
      <div class="nav-section">
        ...
        <a href="cropping" class="nav-link active"><i>🌾</i><span>Cropping Patterns</span></a>
  ...
  <!-- Main Content -->
  <div class="main">
    <h1>Cropping Patterns</h1>
    <p>Provide details about your cropping patterns below for analysis.</p>

    <!-- Button to Toggle Form -->
    <button class="toggle-button" onclick="toggleForm()">Add Cropping Pattern</button>
    <a href="{{ route('cropping.export') }}" class="btn btn-primary">Export CSV</a>



    <form method="GET" action="{{ url('cropping') }}" style="margin-top: 20px; margin-bottom: 20px; display: flex; gap: 20px; flex-wrap: wrap; align-items: flex-end;">
  
  <div class="form-group" style="flex: 1 1 250px;">
    <label for="locality">Filter by Locality</label>
    <input type="text" name="locality" id="locality" value="{{ request('locality') }}" placeholder="Enter locality" style="width: 100%; padding: 10px; box-sizing: border-box;">
  </div>
  
  <div class="form-group" style="flex: 1 1 250px;">
    <label for="cropType">Filter by Crop Type</label>
    <input type="text" name="cropType" id="cropType" value="{{ request('cropType') }}" placeholder="Enter crop type" style="width: 100%; padding: 10px; box-sizing: border-box;">
  </div>
  ...
  <div class="form-group" style="flex: 0 0 auto; display: flex; align-items: flex-end;">
    ...
    <div class="form-container" id="cropForm">
      ...
      <h2>Cropping Pattern Form</h2>
    <form action="{{ url('/cropping') }}" method="POST">
      @csrf
    ...
    @foreach($croppingPatterns as $pattern)
        <tr>
            <td>{{ $pattern->locality }}</td>
            <td>{{ $pattern->crop_type }}</td>
            <td>{{ $pattern->season }}</td>
            <td>{{ $pattern->irrigation }}</td>
        </tr>
    @endforeach
</tbody>
  ...
  <script>
    function toggleForm() {
      var formContainer = document.getElementById('cropForm');
